/**
 * InformationChannel-Push 配置编辑器核心逻辑 (Based on TrendRadar)
...
// 官网配置文件 URL
const REMOTE_CONFIG_URL = 'https://raw.githubusercontent.com/Asterismking/InformationChannel-Push/refs/heads/master/config/config.yaml';
const REMOTE_FREQUENCY_URL = 'https://raw.githubusercontent.com/Asterismking/InformationChannel-Push/refs/heads/master/config/frequency_words.txt';
const REMOTE_TIMELINE_URL = 'https://raw.githubusercontent.com/Asterismking/InformationChannel-Push/refs/heads/master/config/timeline.yaml';
const REMOTE_VERSION_URL = 'https://raw.githubusercontent.com/Asterismking/InformationChannel-Push/refs/heads/master/version_configs';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 首次访问自动加载仓库配置
    const hasConfig = localStorage.getItem(STORAGE_KEY_CONFIG);
    const hasFrequency = localStorage.getItem(STORAGE_KEY_FREQUENCY);
    const hasTimeline = localStorage.getItem(STORAGE_KEY_TIMELINE);
    if (!hasConfig && !hasFrequency && !hasTimeline) {
        autoLoadRemoteConfig();
    }
});

// 首次访问自动加载远程仓库配置
async function autoLoadRemoteConfig() {
    showToast('首次访问，正在从仓库加载默认配置...', 'info');
    try {
        const [configRes, frequencyRes, timelineRes] = await Promise.all([
            fetch(REMOTE_CONFIG_URL),
            fetch(REMOTE_FREQUENCY_URL),
            fetch(REMOTE_TIMELINE_URL)
        ]);
        ...
        if (configRes.ok) {
            ...
            try {
                ...
                console.warn('Config YAML parse error:', e);
            }
        }

        if (frequencyRes.ok) {
            const text = await frequencyRes.text();
        ...
        if (timelineRes.ok) {
            ...
            try {
                ...
                console.warn('Timeline YAML parse error:', e);
            }
        }

        saveAllToLocalStorage();
        showToast('已加载仓库默认配置', 'success');
    } catch (err) {
        console.error('自动加载配置失败:', err);
        showToast('自动加载失败，请手动点击"从仓库加载最新配置"', 'warning');
    }
}
    ...
    modal.innerHTML = `
        <div class="modal-content" style="max-width: 420px;">
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-lg font-bold text-gray-800"><i class="fa-solid fa-cloud-arrow-down mr-2 text-blue-500"></i>从仓库加载最新配置</h3>
            ...
            <div class="text-xs text-gray-400 mt-3 p-2 bg-gray-50 rounded">
                ...
                数据来源：<a href="https://github.com/Asterismking/InformationChannel-Push" target="_blank" class="text-blue-500 hover:underline">Asterismking/InformationChannel-Push</a>
            </div>
    ...
    switch(mod.key) {
        case "platforms":
            ...
            html += `<div class="flex items-center gap-2 mt-3">
                        ...
                        <a href="https://github.com/Asterismking/InformationChannel-Push" target="_blank" class="text-xs bg-gray-100 text-gray-600 px-3 py-1.5 rounded hover:bg-gray-200 transition-colors border border-gray-200 flex items-center gap-1 no-underline">
...
function buildFrequencyText(data) {
    ...
    // 如果没有原始文本，使用默认模板
    let text = '# ═══════════════════════════════════════════════════════════════\n';
    text += '#                    InformationChannel-Push 频率词配置文件\n';
    text += '# ═══════════════════════════════════════════════════════════════\n\n';
